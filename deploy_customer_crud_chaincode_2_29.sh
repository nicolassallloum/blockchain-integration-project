...
CHAINCODE_VERSION="${CHAINCODE_VERSION:-2.29}"
CHAINCODE_SEQUENCE="${CHAINCODE_SEQUENCE:-29}"
...
EVIDENCE_DIR="$PROJECT_ROOT/deployment-evidence/customer-crud-chaincode-${CHAINCODE_VERSION}-${STAMP}"
...
[[ "$CHAINCODE_VERSION" == "2.29" ]] \
  || fail "This controlled deployment requires version 2.29."

[[ "$CHAINCODE_SEQUENCE" == "29" ]] \
  || fail "This controlled deployment requires sequence 29."
...
section "8. COMMIT VERSION 2.29 SEQUENCE 29"
...
echo "Evidence Directory:  $EVIDENCE_DIR"
echo
echo "UpdateResident and DeleteResident are now part of the committed chaincode definition."
