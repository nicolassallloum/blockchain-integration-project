...
async function resolveValidBlockchainOrganization(client, organizationId, organizationCode) {
  if (!organizationId) {
    return { organizationId: null, organizationCode: null };
  }

  // Source of truth in this project is blockchain.blockchain_organization.
  // The old blockchain.organizations table may still exist and may still be referenced
  // by the current FK on blockchain.transactions.organization_id. To avoid FK failures,
  // we read from blockchain.blockchain_organization and, when needed, mirror a minimal
  // organization row into blockchain.organizations before inserting the transaction.
  const hasBlockchainOrganization = await tableExists(client, 'blockchain', 'blockchain_organization');
  const hasLegacyOrganizations = await tableExists(client, 'blockchain', 'organizations');

  let sourceRow = null;

  if (hasBlockchainOrganization) {
    const columns = await getColumns(client, 'blockchain', 'blockchain_organization');
    ...
    if (idColumn) {
      const result = await client.query(
        ...
        SELECT
          ...
          ${codeColumn ? codeColumn : 'NULL'} AS organization_code,
          ${nameColumn ? nameColumn : 'NULL'} AS organization_name
        FROM blockchain.blockchain_organization
        WHERE ${idColumn}::text = $1::text
        LIMIT 1
        `,
        [String(organizationId)]
      );

      sourceRow = result.rows[0] || null;
    }
  }

  if (!sourceRow && hasLegacyOrganizations) {
    const result = await client.query(
      `
      SELECT organization_id::text AS organization_id, organization_code, organization_name
      FROM blockchain.blockchain_organization
  ...
  if (hasLegacyOrganizations) {
    ...
    if (!legacyCheck.rows[0]) {
      const legacyColumns = await getColumns(client, 'blockchain', 'organizations');
      const insertColumns = [];
      const values = [];
      const params = [];
      ...
      push('organization_id', sourceRow.organization_id);
      push('organization_code', organizationCode || sourceRow.organization_code || null);
      push('organization_name', sourceRow.organization_name || 'Generated Organization');
      push('name', sourceRow.organization_name || 'Generated Organization');
      push('organization_type', 'OTHER');
      push('status', 'ACTIVE');
      ...
      if (insertColumns.length > 0) {
        await client.query(
          `
          INSERT INTO blockchain.organizations (${insertColumns.join(', ')})
          VALUES (${values.join(', ')})
  ...
  return {
    ...
    organizationCode: organizationCode || sourceRow.organization_code || null
  };
}

async function insertBlockchainTransaction(client, transactionData) {
  const validOrganization = await resolveValidBlockchainOrganization(
    client,
    transactionData.organizationId,
    transactionData.organizationCode
  );
  ...
  const result = await client.query(
    ...
    [
      ...
      validOrganization.organizationId,
      validOrganization.organizationCode,
