...
const PUBLIC_ADMINISTRATION_WALLET_CURRENCY = 'GOV';

function forcePublicAdministrationGovCurrency(inputCurrency) {
  const normalized = String(inputCurrency || '').trim().toUpperCase();

  if (normalized && normalized !== PUBLIC_ADMINISTRATION_WALLET_CURRENCY) {
    console.warn(
      `[PUBLIC ADMINISTRATION GOV CURRENCY ENFORCED] Received currency "${inputCurrency}", forced to GOV`
    );
  }

  return PUBLIC_ADMINISTRATION_WALLET_CURRENCY;
}
...
function normalizeAdministrationPayload(body) {
  ...
  return {
    ...
    walletCurrency: forcePublicAdministrationGovCurrency(administration.walletCurrency),
...
async function saveAdministrationToBlockchain(payload, context = {}) {
  const blockchainPayload = {
    ...payload,
    walletCurrency: PUBLIC_ADMINISTRATION_WALLET_CURRENCY,
    currency: PUBLIC_ADMINISTRATION_WALLET_CURRENCY,
    currencyCode: PUBLIC_ADMINISTRATION_WALLET_CURRENCY,
    currency_code: PUBLIC_ADMINISTRATION_WALLET_CURRENCY,
...
async function createPublicAdministration(req, res) {
  try {
    ...
    */
    ...
    payload.walletCurrency = forcePublicAdministrationGovCurrency(
      blockchain.walletCurrency || payload.walletCurrency
    );
    payload.walletStatus =
...
async function createPublicAdministrationWallet(req, res) {
  try {
    ...
    const walletCurrency = forcePublicAdministrationGovCurrency(payload.walletCurrency);
...
async function bulkUploadPublicAdministrations(req, res) {
  try {
    ...
    for (let index = 0; index < rows.length; index++) {
      const payload = normalizeAdministrationPayload(rows[index]);
...
async function getNextPublicAdministrationCodes(req, res) {
  try {
    const result = await pool.query(`
      SELECT nextval('blockchain.public_administration_seq') AS next_number
    `);
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        /*
          This is only a temporary UI value.
          Final wallet address is generated by blockchain after save.
        */
