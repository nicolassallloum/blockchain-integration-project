...
/**
 * Step 20 — Implement Transaction Data History
 *
 * PostgreSQL remains the source of truth.
 * Blockchain stores proof only.
 *
 * This service creates PostgreSQL history rows for TRANSACTION proof records.
 *
 * It must not store or return:
 * - raw PostgreSQL transaction rows
...
const TRANSACTION_SOURCE_VIEW_CANDIDATES = [
  process.env.BLOCKCHAIN_PROOF_TRANSACTION_SOURCE_VIEW,
  'transaction_data_sync',
  'transactions_sync',
  'transaction_sync',
  'valoores_transaction_data_sync',
  'valoores_transactions_sync',
  'government_transactions_sync',
  'government_transaction_data_sync',
  'payment_transactions_sync',
  'payments_sync',
  'transfers_sync',
  'transactions',
...
const TRANSACTION_SOURCE_ID_CANDIDATE_GROUPS = [
  ['transaction_id'],
  ['transaction_code'],
  ['transaction_no'],
  ['transaction_number'],
...
function resolvePrimaryKeyColumns(columns) {
  ...
  for (const group of TRANSACTION_SOURCE_ID_CANDIDATE_GROUPS) {
...
async function discoverTransactionSourceViews() {
  const query = resolveQueryClient();
  ...
  const result = await query(
    ...
    ORDER BY
      ...
      table_name
    `,
    [BLOCKCHAIN_SCHEMA, TRANSACTION_SOURCE_VIEW_CANDIDATES]
  );
  ...
  return {
    ...
    configuredCandidateViews: TRANSACTION_SOURCE_VIEW_CANDIDATES,
    ...
    securityPolicy: {
      ...
      transactionPayloadStored: false,
      hashOnlyStored: true
    }
  };
}

async function getSelectedTransactionSourceView() {
  const discovery = await discoverTransactionSourceViews();

  if (!discovery.selectedView) {
    throw new Error(
      'No usable transaction source view found in schema blockchain. Create or configure BLOCKCHAIN_PROOF_TRANSACTION_SOURCE_VIEW with a transaction_id/transaction_code/client_transaction_id/payment_id/id column.'
    );
  }
...
async function getTransactionSourceCount() {
  const query = resolveQueryClient();
  const selectedView = await getSelectedTransactionSourceView();
...
async function getTransactionSourceRows(limit = DEFAULT_LIMIT) {
  const query = resolveQueryClient();
  const selectedView = await getSelectedTransactionSourceView();
...
async function buildTransactionHistoryDecision(rowContext, row) {
...
async function createSyncRun(runId, submittedBy, sourceViewName) {
  ...
  const valuesByColumn = {
    ...
    error_message: null,
    metadata: safeJson({
      integrationStep: 'STEP_20_TRANSACTION_DATA_HISTORY',
      recordType: RECORD_TYPE,
...
async function previewTransactionHistorySync(options = {}) {
  const limit = normalizeLimit(options.limit);
  const rowContext = await getTransactionSourceRows(limit);
  const decisions = [];

  for (const row of rowContext.rows) {
    decisions.push(await buildTransactionHistoryDecision(rowContext, row));
  }
...
async function syncTransactionHistory(options = {}) {
  const limit = normalizeLimit(options.limit);
  const dryRun = normalizeDryRun(options.dryRun);
  const submittedBy = options.submittedBy || 'STEP_20_TRANSACTION_HISTORY_API';

  const sourceCount = await getTransactionSourceCount();
  const rowContext = await getTransactionSourceRows(limit);
  const decisions = [];
...
module.exports = {
  discoverTransactionSourceViews,
  getTransactionSourceCount,
  previewTransactionHistorySync,
