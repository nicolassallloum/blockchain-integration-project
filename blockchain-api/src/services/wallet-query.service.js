...
class WalletQueryService {
  async getWalletByAddress(walletAddress, requestId) {
    ...
    try {
      ...
      const parsedWallet = this.safeParseFabricResult(fabricResult);

      if (parsedWallet) {
        return {
          success: true,
          httpStatus: 200,
          message: "Wallet details retrieved successfully from blockchain",
          source: "FABRIC",
          data: this.normalizeWallet(parsedWallet),
        };
      }
    } catch (error) {
      console.warn(
        `[${requestId}] Fabric wallet query failed. Falling back to PostgreSQL.`,
        error.message
      );
    }
    ...
    try {
      ...
      if (parsedBalance !== null && parsedBalance !== undefined) {
        return {
          success: true,
          httpStatus: 200,
          message: "Wallet balance retrieved successfully from blockchain",
          source: "FABRIC",
          data: this.normalizeBalance(cleanWalletAddress, parsedBalance),
        };
      }
    } catch (error) {
      console.warn(
        `[${requestId}] Fabric balance query failed. Falling back to PostgreSQL.`,
        error.message
      );
    }
  ...
  async getWalletHistory(walletAddress, options = {}, requestId) {
    ...
    try {
      ...
      const parsedHistory = this.safeParseFabricResult(fabricResult);

      if (Array.isArray(parsedHistory)) {
        return {
          success: true,
          httpStatus: 200,
          message: "Wallet transaction history retrieved successfully from blockchain",
          source: "FABRIC",
          data: {
      ...
      console.warn(
        `[${requestId}] Fabric wallet history query failed. Falling back to PostgreSQL.`,
        error.message
      );
    }
  ...
  async getWalletBalanceFromDatabase(walletAddress) {
    ...
    /*
    * STEP 25 FIX
    * Support multiple possible transaction table names because your project
    * has used both:
    *
    ...
    */
    ...
    if (!tableCheckResult.rows.length) {
        console.warn(
        `No transaction table found in schema blockchain. Returning empty history for wallet ${walletAddress}`
        );
    ...
    /*
    * Detect available columns because schemas may differ between:
    * blockchain.transactions
    * blockchain.blockchain_transaction
    */
    ...
    if (hasFromWalletAddress && hasToWalletAddress) {
        ...
        console.warn(
        `Transaction table ${fullTableName} does not have wallet address columns. Returning empty history.`
        );
