...
It only lists available views and columns.
*/

-- 1. List views in important schemas
SELECT
    table_schema,
    table_name
...
-- 2. Inspect expected AML source view
SELECT
    table_schema,
    table_name,
    column_name,
...
-- 3. Check if expected AML source view exists
SELECT
    CASE
        WHEN EXISTS (
            SELECT 1
...
-- 4. Count AML source records if view exists
DO $$
DECLARE
    v_exists BOOLEAN;
    v_count BIGINT;
